...
bool _abb_guardar(abb_t *arbol, nodo_t* nodo, nodo_t** apuntado, const char *clave, void *dato){
    if(!nodo){
...
bool abb_guardar(abb_t *arbol, const char *clave, void *dato){
    ...
    if(arbol->cmp(clave, arbol->raiz->clave) < 0) return _abb_guardar(arbol, arbol->raiz->izq, &arbol->raiz->izq, clave, dato);
    if(arbol->cmp(clave, arbol->raiz->clave) > 0) return _abb_guardar(arbol, arbol->raiz->der, &arbol->raiz->der, clave, dato);
