...
void obtener_posicion_hash(const hash_t *hash, size_t* inicio, const char* clave){
    while(hash->tabla_hash[*inicio].estado != VACIO){
        if(hash->tabla_hash[*inicio].estado == OCUPADO && strcmp(clave, hash->tabla_hash[*inicio].clave) == 0){  
            break;
        }
        *inicio += 1;
        if(*inicio >= hash->capacidad){
            *inicio = 0;
        }
    }
}
...
bool hash_guardar(hash_t *hash, const char *clave, void *dato){
    ...
    obtener_posicion_hash(hash, &pos, copia);
...
void *hash_borrar(hash_t *hash, const char *clave){
    ...
    obtener_posicion_hash(hash, &pos, clave);
...
bool hash_pertenece(const hash_t *hash, const char *clave){
    ...
    if(hash->tabla_hash[pos].estado == VACIO || hash->tabla_hash[pos].estado == BORRADO){
        return false;
    }
    return true;
}
